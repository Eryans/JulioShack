...
const Imagehandler = ({ image, refresh, allowOptionForm = false }) => {
	...
	const canModify = useMemo(
		() => (auth ? image.user === auth._id : false),
		[auth, image.user]
	);
	...
	const handleSubmit = (event) => {
		try {
			...
			setUserImagePrivacy(auth._id, image._id, dataToSubmit).then(
				(res) => {
					if (res.success) {
					...
					>
						<Form onSubmit={handleSubmit} className="w-100">
							<Form.Group controlId="isPrivateCheckbox">
								...
								{auth?.profilePic?.path === image.path && (
									<p>
										La photo de profile ne peut pas être
										privée
									</p>
								)}
							</Form.Group>
							...
							<span className="d-flex justify-content-start align-items-start gap-2">
								...
								<DeleteImageButton
									imageId={image._id}
									refresh={refresh}
									handleCloseModalOrigin={handleCloseModal}
								/>
							</span>
						</Form>
					</Modal.Footer>
